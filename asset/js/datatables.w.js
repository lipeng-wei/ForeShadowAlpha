 ...
 *   https://datatables.net/download/#dt/dt-1.10.10,b-1.1.0,b-colvis-1.1.0,cr-1.3.0,fc-3.2.0,fh-3.1.0,kt-2.1.0,r-2.0.0,rr-1.1.0,sc-1.4.0,se-1.1.0
 *
 * Included libraries:
 *   DataTables 1.10.10, Buttons 1.1.0, Column visibility 1.1.0, ColReorder 1.3.0, FixedColumns 3.2.0, FixedHeader 3.1.0, KeyTable 2.1.0, Responsive 2.0.0, RowReorder 1.1.0, Scroller 1.4.0, Select 1.1.0
 */
...
(function( $, window, document, undefined ) {
	...
	function _fnBrowserDetect( settings )
	{
		...
		settings.oScroll.iBarWidth = 0;//DataTable.__browser.barWidth; edit by lipeng_wei
	}
	...
	_api_registerPlural( 'cells().nodes()', 'cell().node()', function () {
		return this.iterator( 'cell', function ( settings, row, column ) {
            ...
            //edit by lipeng_wei
            //新增 解决 删除某行之后 cell的row越界问题
            if ( ! settings.aoData[ row ]) return undefined;
	...
	DataTable.defaults = {
		...
		"fnStateLoadCallback": function ( settings ) {
			try {
				return JSON.parse(
					...
					/* begin edit by lipeng_wei

					'DataTables_'+settings.sInstance+'_'+location.pathname,

					*/

                    settings.oInit.foreTableI+'_DataTables_'+settings.sInstance+'_'+location.pathname+location.search

					/* end edit by lipeng_wei
					*/
					)
				);
			} catch (e) {}
		...
		"oLanguage": {
			 ...
			 *  @type string
			 *  @default (Searched from _MAX_ total entries)
			 *
			 *  @dtopt Language
			 ...
			 *    } );
			 */
			"sInfoFiltered": '<' + " Searched from _MAX_ entries",
	...
	DataTable.ext = _ext = {
		 ...
		 *  @type string
		 */
		build:"dt/dt-1.10.10,b-1.1.0,b-colvis-1.1.0,cr-1.3.0,fc-3.2.0,fh-3.1.0,kt-2.1.0,r-2.0.0,rr-1.1.0,sc-1.4.0,se-1.1.0",
...
$.extend( KeyTable.prototype, {
	...
	_focus: function ( row, column )
	{
        ...
        //console.info(lastFocus);
        //var contest = lastFocus? lastFocus.node(): 'none';
        //console.info('~', contest);
...
function info ( api )
{
    ...
    api.rows( { selected: true } ).flatten().length &&
    add( 'row',    api.rows( { selected: true } ).flatten().length );
    api.columns( { selected: true } ).flatten().length &&
    add( 'column', api.columns( { selected: true } ).flatten().length );
    api.cells( { selected: true } ).flatten().length &&
